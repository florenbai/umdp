...
func (ww *weWork) KfAccountDel(corpId uint, kfId string) (resp BizResponse) {
	if kfId == "" {
		resp.ErrCode = 500
...
func (ww *weWork) KfAccountUpdate(corpId uint, account KfAccount) (resp BizResponse) {
	if ok := validate.Struct(account); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) KfUpgradeService(corpId uint, request UpgradeServiceRequest) (resp BizResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) KfCancelUpgradeService(corpId uint, request CancelUpgradeServiceRequest) (resp BizResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
