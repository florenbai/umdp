...
func (ww *weWork) ExternalUpdateContactWay(corpId uint, me ContactMe) (resp BizResponse) {
	if ok := validate.Struct(me); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) ExternalDeleteContactWay(corpId uint, configId string) (resp BizResponse) {
	p := H{"config_id": configId}
	_, err := ww.getRequest(corpId).SetBody(p).SetResult(&resp).
...
func (ww *weWork) ExternalCloseTempChat(corpId uint, userId, externalUserId string) (resp BizResponse) {
