...
func (ww *weWork) ActiveAccount(request ActiveAccountRequest) (resp BizResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) SetAutoActiveStatus(request SetAutoActiveStatusRequest) (resp BizResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
