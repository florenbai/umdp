...
func (ww *weWork) UpdateProductAlbum(corpId uint, request ProductUpdateRequest) (resp BizResponse) {
...
func (ww *weWork) DeleteProductAlbum(corpId uint, productId string) (resp BizResponse) {
	h := H{}
	h["product_id"] = productId
	_, err := ww.getRequest(corpId).SetBody(h).SetResult(&resp).
