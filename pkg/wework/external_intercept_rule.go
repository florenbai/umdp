...
func (ww *weWork) UpdateInterceptRule(corpId uint, request UpdateInterceptRuleRequest) (resp BizResponse) {
...
func (ww *weWork) DeleteInterceptRule(corpId uint, ruleId string) (resp BizResponse) {
	h := H{}
	h["rule_id"] = ruleId
	_, err := ww.getRequest(corpId).SetBody(h).SetResult(&resp).
