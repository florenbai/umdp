...
func (ww *weWork) UpdateInvoiceStatus(corpId uint, request UpdateInvoiceStatusRequest) (resp BizResponse) {
...
func (ww *weWork) UpdateInvoiceStatusBatch(corpId uint, request UpdateInvoiceStatusBatchRequest) (resp BizResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
