...
func (ww *weWork) SetArchSyncMode(corpId uint, mode int) (resp BizResponse) {
	h := H{}
...
func (ww *weWork) SetSubScribeMode(corpId uint, mode int) (resp BizResponse) {
	h := H{}
...
func (ww *weWork) SetTeacherViewMode(corpId uint, mode int) (resp BizResponse) {
	h := H{}
	h["view_mode"] = mode
	_, err := ww.getRequest(corpId).SetBody(h).SetResult(&resp).
