...
func (ww *weWork) CorpTagUpdate(corpId uint, tag CorpTag) (resp BizResponse) {
	_, err := ww.getRequest(corpId).
...
func (ww *weWork) CorpTagDelete(corpId uint, tagIds, groupIds []string) (resp BizResponse) {
	p := H{"tag_id": tagIds, "group_id": groupIds}
	_, err := ww.getRequest(corpId).
		SetBody(p).SetResult(&resp).
...
func (ww *weWork) MarkTag(corpId uint, userId string, externalUserId string, addTag []int, removeTag []int) (resp BizResponse) {
