...
func (ww *weWork) RemindGroupMsgSend(corpId uint, msgid string) (resp BizResponse) {
	h := H{}
	h["msgid"] = msgid
...
func (ww *weWork) CancelGroupMsgSend(corpId uint, msgId string) (resp BizResponse) {
	h := H{}
	h["msgid"] = msgId
	_, err := ww.getRequest(corpId).SetBody(h).SetResult(&resp).
