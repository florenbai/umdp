...
// UserCreate 创建成员
func (ww *weWork) UserCreate(corpId uint, user User) (resp BizResponse) {
...
// UserUpdate 更新成员
func (ww *weWork) UserUpdate(corpId uint, user User) (resp BizResponse) {
...
// UserDelete 删除成员
func (ww *weWork) UserDelete(corpId uint, userId string) (resp BizResponse) {
	_, err := ww.getRequest(corpId).SetQueryParam("userid", userId).SetResult(&resp).
...
func (ww *weWork) UserSimpleList(corpId uint, departId int32, fetchChild int) (resp UserSimpleListResponse) {
	if departId <= 0 {
		return UserSimpleListResponse{Error{ErrorMsg: "部门ID必需大于0", ErrCode: 403}, nil}
	}
